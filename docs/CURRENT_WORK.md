...
| 当前任务编号 | ##043 remove-daily-checkin（已完成） |
| 状态 | ##043 已完成：每日回访功能（daily_checkins / CheckinCard）全部删除——代码 9 个文件清理、flow-data-map 删除、原型 4 函数删除、文档架构债「修 3」清除。typecheck 通过，目标测试绿。下一步：触发「进入开发」门控 → Phase 1 AI 服务层脚手架（T2-04a） |
...
| 更新时间 | 2026-05-25 |
| 负责人 | Claude |
| 任务编号 | ##019 blueprint-stage-independence-and-cross-cutting-insight |
| 状态 | DONE：已在 `docs/02-master-blueprint.md` 固化两条上位原则——**阶段独立完整性**（每个 Phase 必须独立成立，不暴露阶段名 / 不显示未来解锁占位 / Schema 不预留未来字段）和**模块独立 + 横切层架构**（6 个纵向独立功能模块 + 身体洞察横切沉淀层；身体洞察的唯一对外表达是 4 块身体拼图）。后续 Phase 1 原型需按新原则修正：① 删除「你的身体说」橙色卡（与块1「我的吃法」重叠）；② 改造首页「身体镜像」卡为身体拼图最新更新通知；③ 推荐理由标签不再暴露 RecommendationStage 名 |
